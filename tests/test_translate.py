...
class TestTranslateTextAuto:
    async def test_no_translator_returns_original(self):
        utils = _get_utils()
        with (
            patch.object(utils, "openai_trans", False),
    ...
    async def test_timeout_returns_original(self):
        ...
        with (
            patch.object(utils, "openai_trans", True),
            patch.object(utils, "openai_translate_text", slow_translate),
        ):
    ...
    async def test_exception_returns_original(self):
        ...
        with (
            patch.object(utils, "openai_trans", True),
            patch.object(utils, "openai_translate_text", failing_translate),
        ):
            result = await utils.translate_text_auto("hello world")
            assert result == "hello world"
    ...
    async def test_success_returns_translated(self):
        ...
        with (
            patch.object(utils, "openai_trans", True),
            patch.object(utils, "openai_translate_text", mock_translate),
        ):
            result = await utils.translate_text_auto("hello world")
            assert result == "你好世界"
